...
<body>
    ...
    $("#fileUpload").fileinput({  
        language : "zh",//设置语言  
        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=zbtxbtsjk",//上传地址  
        textEncoding: 'UTF-8',
		...
		ajax : function (request) {
			...
			columns: [{
				...
				editable: {  
					...
					type: 'select',
                    source: [                 	
                    	<c:forEach items="${listZHGLZBSX}" var="item">
                			{value:"${item.zhglzbsx}",text:"${item.zhglzbsx}"},
						</c:forEach>           	
                    ]                      
				}
			},{
			 ...
			 onEditableSave: function (field, row, oldValue, $el) {
				 	...
				 	if(field == "rydm") {
						...
						// var jsonZHGLZBSX = JSON.stringify(${listZHGLZBSX});					 
						 //alert(json);
						 <c:forEach items="${listZHGLZBSX}" var="item">
                			//{value:"${item.zhglzbsx}",text:"${item.zhglzbsx}"},
                			
                			if("${item.zhglzbsx}".trim() == row.nx.trim()) {
                				jsonSJ[row.rowid].jlje = "${item.jlje}";
                			}
						</c:forEach>  					
						 data : jsonSJ;
						 $.ajax({
			...
			window.operateEvents = {
					 'click .save': function (e, value, row, index) {
						 ...
						 $.ajax({
			                    type: "post",
			                    url: "../controller/ZBTXBTSJKController.jsp?op=update",
			 ...
			 'click .delete': function (e, value, row, index) {
				 ...
				 $.ajax({
	                   type: "post",
	                   url: "../controller/ZBTXBTSJKController.jsp?op=delete",
	                   data: {"ID": row.id},
	                   dataType: 'JSON',
	                   success: function (data, status) {
	                       if (status == "success") {
