...
<script>
		<%
			...
			if(op == null || op == ""){
				op = "getAll";				
				url = "../controller/JXGZLSJKController.jsp?op=getAll";
			}
			else if(op.equals("personal"))
				url = "../controller/JXGZLSJKController.jsp?op=personal";
		...
		<!-- 设置上传相关参数 -->
   		<script>  
		    $("#fileUpload").fileinput({  
		        language : "zh",//设置语言  
		        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=jxgzlsjk",//上传地址  
		        textEncoding: 'UTF-8',
		...
		ajax : function (request) {
			...
			columns: [{
			 ...
			 onEditableSave: function (field, row, oldValue, $el) {
					if(field == "rydm") {
						 ...
						 row.gzlhj = row.zwgzl + row.bky + row.bkqty + row.bke + row.bkqte + row.bktk 
						 + row.yjsjxgzly + row.yjszdgzly + row.yjsjxgzle + row.yjszdgzle;
					}
			...
			window.operateEvents = {
					 'click .save': function (e, value, row, index) {
						 ...
						 $.ajax({
			                    type: "post",
			                    url: "../controller/JXGZLSJKController.jsp?op=update",
			 ...
			 'click .delete': function (e, value, row, index) {
				 ...
				 $.ajax({
	                   type: "post",
	                   url: "../controller/JXGZLSJKController.jsp?op=delete",
	                   data: {"ID": row.id},
	                   dataType: 'JSON',
	                   success: function (data, status) {
	                       if (status == "success") {
