...
<script src="../static/js/bootstrap/js/bootstrap-table-zh-CN.js"></script>
	...
	<script>
		<%
			...
			if(op == null || op == ""){
				op = "getAll";				
				url = "../controller/JYSGZTJBController.jsp?op=getAll";
			}
			else if(op.equals("personal"))
				url = "../controller/JYSGZTJBController.jsp?op=personal";
...
<body>
    ...
    <c:if test="${yh.getYHGROUP() == 0 || yh.getYHGROUP() == 1}">  
		...
		<script>
			$("#btn_add").click(function () {
				 var index = $('#table').bootstrapTable().length;
			            ...
			            index: 0,
			            row: {jysmc:'-',jcxm:'-',qykcxm:'-',sqxxxm:'-',cxktxm:'-',zhggxm:'-',jxmscxxm:'-',
			            	sxddjs:'-',wksj:'-',hj:'-'}
			        })
			})
		</script>
	    ...
	    $("#fileUpload").fileinput({  
	        language : "zh",//设置语言  
	        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=jysgztjb",//上传地址  
	        textEncoding: 'UTF-8',
		...
		ajax : function (request) {
			...
			window.operateEvents = {
					 'click .save': function (e, value, row, index) {
						 ...
						 $.ajax({
			                    type: "post",
			                    url: "../controller/JYSGZTJBController.jsp?op=update",
			 ...
			 'click .delete': function (e, value, row, index) {
				 ...
				 $.ajax({
	                   type: "post",
	                   url: "../controller/JYSGZTJBController.jsp?op=delete",
	                   data: {"ID": row.id},
	                   dataType: 'JSON',
	                   success: function (data, status) {
	                       if (status == "success") {
