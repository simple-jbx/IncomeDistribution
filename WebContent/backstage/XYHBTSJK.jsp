...
<body>
	...
	<script>
		$("#btn_add").click(function () {
			 var index = $('#table').bootstrapTable().length;
		            ...
		            index: 0,
		            row: {rydm:'-',xm:'-',xyhbz:'0.00',je:'0.00'}
		        })
		})
	</script>
	<script>  
    $("#fileUpload").fileinput({  
        language : "zh",//设置语言  
        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=zyhbtsjk",//上传地址  
		...
		ajax : function (request) {
			...
			window.operateEvents = {
					 'click .save': function (e, value, row, index) {
						 ...
						 $.ajax({
			                    type: "post",
			                    url: "../controller/XYHBTSJKController.jsp?op=update",
			 ...
			 'click .delete': function (e, value, row, index) {
				 ...
				 $.ajax({
	                   type: "post",
	                   url: "../controller/XYHBTSJKController.jsp?op=delete",
	                   data: {"ID": row.id},
	                   dataType: 'JSON',
	                   success: function (data, status) {
	                       if (status == "success") {
