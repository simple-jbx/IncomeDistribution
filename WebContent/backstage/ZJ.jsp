...
<body>
    ...
    <c:if test="${yh.getYHGROUP() == 0 || yh.getYHGROUP() == 1}">  
	    ...
	    $("#fileUpload").fileinput({  
	        language : "zh",//设置语言  
	        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=zj",//上传地址  
	        textEncoding: 'UTF-8',
	...
	$(function(){		
		...
		ajax : function (request) {
	        $.ajax({
	            type : "GET",
	            url : "../controller/ZJController.jsp?op=getAll",
		 ...
		 window.operateEvents = {
				 'click .save': function (e, value, row, index) {
					 ...
					 $.ajax({
		                    type: "post",
		                    url: "../controller/ZJController.jsp?op=update",
		 ...
		 'click .delete': function (e, value, row, index) {
			 ...
			 $.ajax({
                    type: "post",
                    url: "../controller/ZJController.jsp?op=delete",
                    data: {"ID": row.id},
                    dataType: 'JSON',
                    success: function (data, status) {
                        if (status == "success") {
