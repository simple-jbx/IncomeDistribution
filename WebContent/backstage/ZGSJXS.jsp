...
<script>
		<%
			...
			if(op == null || op == ""){
				op = "getAll";				
				url = "../controller/ZGSJXSController.jsp?op=getAll";
			}
			else if(op.equals("personal"))
				url = "../controller/ZGSJXSController.jsp?op=personal";
    	...
    	<c:if test="${yh.getYHGROUP() == 0 || yh.getYHGROUP() == 1}">  
			...
			<script>
				$("#btn_add").click(function () {
				            ...
				            index: 0,
				            row: {rydm:'-',xm:'-',zgys:'0',zgxs:'0',zgqksm:'-'}
				        })
				})
			</script>
		...
		ajax : function (request) {
		 ...
		 window.operateEvents = {
			'click .save': function (e, value, row, index) {
				 $.ajax({
		                 type: "post",
		                 url: "../controller/ZGSJXSController.jsp?op=update",
		 ...
		 'click .delete': function (e, value, row, index) {
			if(window.confirm("确定删除?")){
				$.ajax({
                    type: "post",
                    url: "../controller/ZGSJXSController.jsp?op=delete",
                    data: {"ID": row.id},
                    dataType: 'JSON',
                    success: function (data, status) {
                        if (status == "success") {
