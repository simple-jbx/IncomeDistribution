...
<script>
		<%
			...
			if(op == null || op == ""){
				op = "getAll";				
				url = "../controller/JKGZSJKController.jsp?op=getAll";
			}
			else if(op.equals("personal"))
				url = "../controller/JKGZSJKController.jsp?op=personal";
...
<body>
    ...
    <c:if test="${yh.getYHGROUP() == 0 || yh.getYHGROUP() == 1}">  
		...
		<script>
		 ...
		 $.ajax({
	         type : "POST",
	         url : "../controller/QTGZController.jsp?op=jkgz",
	    ...
	    $("#fileUpload").fileinput({  
	        language : "zh",//设置语言  
	        uploadUrl: "${pageContext.request.contextPath}/upLoadServlet?tableName=jkgzsjk",//上传地址  
		...
		ajax : function (request) {
			...
			window.operateEvents = {
					 'click .save': function (e, value, row, index) {
						 $.ajax({
			                    type: "post",
			                    url: "../controller/JKGZSJKController.jsp?op=update",
			 ...
			 'click .delete': function (e, value, row, index) {
				 ...
				 $.ajax({
	                   type: "post",
	                   url: "../controller/JKGZSJKController.jsp?op=delete",
	                   data: {"ID": row.id},
	                   dataType: 'JSON',
	                   success: function (data, status) {
	                       if (status == "success") {
