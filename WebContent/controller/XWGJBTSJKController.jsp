...
<%
	...
	String personalRefererURL = basePath + "/personal/XWGJBTSJK.jsp";
	String mamagerReffererURL = basePath + "/backstage/basicmanage/XWGJBTSJK.jsp";
	if(paraYH == null) {
	...
	if(StringUtils.isEmpty(paraOp)) {
		...
		XWGJBTSJK outObject = commonService.getDataByRydm(XWGJBTSJK.class, paraYH.getRYDM(), paraYear);
		if(outObject == null)
			outObject = new XWGJBTSJK();
		out.print(outObject.toJSON());
		return;
	}else {
		 int authority = AuthorityIdentify.authIdentify(paraYH, "XWGJBTSJK");
		 if(paraOp.equals("getAll") && authority > 0) {
			 String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(XWGJBTSJK.class, paraYear);
				if(dataList != null) 
			...
			if(paraOp.equals("update")) {
				...
				if(StringUtils.isEmpty(row)) {
					...
					XWGJBTSJKService.updateData(JSON2Object);
					out.println("1");
				}	
