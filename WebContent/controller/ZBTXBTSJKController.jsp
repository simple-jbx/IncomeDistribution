<%@ page language="java"
	import="service.ZBTXBTSJKService,service.commonService,utils.StringUtils,
	bean.YH,bean.ZBTXBTSJK,java.util.List,java.util.Map,utils.List2JsonUtils,
...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			ZBTXBTSJK outObject = commonService.getDataByRydm(ZBTXBTSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new ZBTXBTSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			ZBTXBTSJKService objectService = new ZBTXBTSJKService();
			if(paraYh.equals("update")) {
				String paraRow = request.getParameter("row");
				if(StringUtils.isEmpty(paraRow)) {
					out.print("-1");//请求数据为空
				}else {
					ZBTXBTSJK JSON2Object = JSON.parseObject(paraRow, ZBTXBTSJK.class);
				...
				if(StringUtils.isEmpty(paraID)) {
					...
					commonService.deleteByID(ZBTXBTSJK.class, paraID);
					out.print("1");					
				}
			}else if(paraOp.equals("getAll")) {
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(ZBTXBTSJK.class);
				if(dataList != null)
