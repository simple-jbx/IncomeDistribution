<%@ page language="java"
	import="service.KYSJKService,service.commonService,utils.StringUtils,
	bean.YH,bean.KYSJK,java.util.List,java.util.Map,utils.List2JsonUtils,
...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			KYSJK outObject = commonService.getDataByRydm(KYSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new KYSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			KYSJKService objectService = new KYSJKService();
			if(paraOp.equals("update")) {
				String row = request.getParameter("row");
				if(StringUtils.isEmpty(row)) {
					out.print("-1");//请求数据为空
				}else {
					KYSJK JSON2Object = JSON.parseObject(row, KYSJK.class);
				...
				if(StringUtils.isEmpty(paraID)) {
					...
					commonService.deleteByID(KYSJK.class, paraID);
					out.print("1");					
				}
			}else if(paraOp.equals("getAll")) {
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(KYSJK.class);
				if(dataList != null) 
