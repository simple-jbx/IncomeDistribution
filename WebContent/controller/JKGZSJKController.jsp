<%@ page language="java"
	import="service.JKGZSJKService,service.commonService,utils.StringUtils,
	bean.YH,bean.JKGZSJK,java.util.List,java.util.Map,utils.List2JsonUtils,
...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			JKGZSJK outObject = commonService.getDataByRydm(JKGZSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new JKGZSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			JKGZSJKService objectService = new JKGZSJKService();
			if(paraOp.equals("update")) {
				String row = request.getParameter("row");
				if(StringUtils.isEmpty(row)) {
					out.print("-1");//请求数据为空
				}else {
					JKGZSJK JSON2Object = JSON.parseObject(row, JKGZSJK.class);
				...
				if(StringUtils.isEmpty(paraID)) {
					...
					commonService.deleteByID(JKGZSJK.class, paraID);
					out.print("1");					
				}
			}else if(paraOp.equals("getAll")) {
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(JKGZSJK.class);
				if(dataList != null) 
