<%@ page language="java" import="service.XYHBTSJKService,service.commonService,utils.StringUtils,
	bean.YH,bean.XYHBTSJK,java.util.List,java.util.Map,utils.List2JsonUtils,com.alibaba.fastjson.JSON,
...
<%
	...
	String personalRefererURL = basePath + "/personal/XYHBTSJK.jsp";
	String mamagerReffererURL = basePath + "/backstage/basicmanage/XYHBTSJK.jsp";
	...
	if(StringUtils.isEmpty(paraOp)) {
		...
		XYHBTSJK outObject = commonService.getDataByRydm(XYHBTSJK.class, paraYH.getRYDM(), paraYear);
		if(outObject == null)
			outObject = new XYHBTSJK();
		out.print(outObject.toJSON());
		return;
	}else {//所有数据的相关操作
		 int authority = AuthorityIdentify.authIdentify(paraYH, "XYHBTSJK");
		 if(paraOp.equals("getAll") && authority > 0) {
			String outJSON = "0";
			List<Map<String, Object> > dataList = commonService.getAllData(XYHBTSJK.class, paraYear);
			if(dataList != null) 
			...
			if(paraOp.equals("single")) {
				if(field.equals("update")) {
					...
					if(StringUtils.isEmpty(row)) {
						...
						XYHBTSJK JSON2Object = JSON.parseObject(row, XYHBTSJK.class);
						XYHBTSJKService.updateData(JSON2Object);
						out.println("1");
					}		
					...
					if(StringUtils.isEmpty(paraID)) {
						...
						commonService.deleteByID(XYHBTSJK.class, paraID);
						out.print("1");					
					}
				}
				...
				if(field.equals("delete")) {
					...
					commonService.deleteByIDList(XYHBTSJK.class, jsonDeleteArray);
					out.print("1");
				}else if(field.equals("update")) {
					List<XYHBTSJK> updateList = JSON.parseArray((String)request.getParameter("list_rows"), XYHBTSJK.class);
					XYHBTSJKService.updateData(updateList);
					out.println("1");
				}
			}else {
