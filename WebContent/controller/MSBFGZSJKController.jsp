...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			MSBFGZSJK outObject = commonService.getDataByRydm(MSBFGZSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new MSBFGZSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			MSBFGZSJKService objectService = new MSBFGZSJKService();
			if(paraOp.equals("update")) {
				String paraRow = request.getParameter("row");
				if(StringUtils.isEmpty(paraRow)) {
					out.print("-1");//请求数据为空
				}else {
					MSBFGZSJK JSON2Object = JSON.parseObject(paraRow, MSBFGZSJK.class);
				...
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(MSBFGZSJK.class);
				if(dataList != null)
