...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			ZJHDBTSJK outObject = commonService.getDataByRydm(ZJHDBTSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new ZJHDBTSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			ZJHDBTSJKService objectService = new ZJHDBTSJKService();
			if(paraOp.equals("update")) {
				String paraRow = request.getParameter("row");
				if(StringUtils.isEmpty(paraRow)) {
					out.print("-1");//请求数据为空
				}else {
					ZJHDBTSJK JSON2Object = JSON.parseObject(paraRow, ZJHDBTSJK.class);
				...
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(ZJHDBTSJK.class);
				if(dataList != null)
