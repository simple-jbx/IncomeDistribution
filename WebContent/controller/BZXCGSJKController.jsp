<%@ page language="java"
	import="service.BZXCGSJKService,service.commonService,utils.StringUtils,
	bean.YH,bean.BZXCGSJK,java.util.List,java.util.Map,utils.List2JsonUtils,
...
<%
	...
	if(paraYh != null) {
		if(StringUtils.isEmpty(paraOp)) {
			...
			BZXCGSJK outObject = commonService.getDataByRydm(BZXCGSJK.class, paraYh.getGH(), paraYear);
			if(outObject == null)
				outObject = new BZXCGSJK();
			out.print(outObject.toJSON());
		}else if(paraYh.getYHGROUP() == 0 || paraYh.getYHGROUP() == 1) {
			BZXCGSJKService objectService = new BZXCGSJKService();
			if(paraOp.equals("update")) {
				String row = request.getParameter("row");
				if(StringUtils.isEmpty(row)) {
					out.print("-1");//请求数据为空
				}else {
					BZXCGSJK JSON2Object = JSON.parseObject(row, BZXCGSJK.class);
				...
				if(StringUtils.isEmpty(paraID)) {
					...
					commonService.deleteByID(BZXCGSJK.class, paraID);
					out.print("1");					
				}
			}else if(paraOp.equals("getAll")) {
				String outJSON = "0";
				List<Map<String, Object> > dataList = commonService.getAllData(BZXCGSJK.class);
				if(dataList != null) 
