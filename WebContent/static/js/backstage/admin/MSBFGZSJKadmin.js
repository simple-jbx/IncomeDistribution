	...
	$("#btn_add").click(function () {
		...
		$('#table').bootstrapTable('insertRow', {
		    index: 0,
		    row: {rydm:defaultValue,xm:defaultValue,msdsgzl:defaultValue,gzlbzj:defaultValue,
		      	ffje:defaultValue,nd:defaultValue
            }
		})
	});
		...
		if(edit == false){
				...
				columns: [
					...
					onEditableSave: function (field, row, oldValue, $el) {
						if(field == "rydm") {
						 ...
						 if(field == "msgzl" || field == "gzlbzj"){	
							if(isNaN(row.msgzl) || isNaN(row.gzlbzj))
								row.ffje = parseFloat(row.msgzl) * parseFloat(row.gzlbzj);
						}
			...
			window.operateEvents = {
				'click .save': function (e, value, row, index) {
					if(row.rydm == "-" || row.xm == "-") {
						alert("该用户不存在");
					}else if(field == "msgzl" || field == "gzlbzj"){	
						if(isNaN(row.msgzl) || isNaN(row.gzlbzj))
							row.ffje = 0;
						else 
							row.ffje = parseFloat(row.msgzl) * parseFloat(row.gzlbzj);
