...
public class upLoadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //消息提示
        StringBuffer message = new StringBuffer("{msg:\"message\"}");
		try{
            ...
            for(FileItem item : list){  
                ...
                if(item.isFormField()){  
                   ...
                   // System.out.println(name + "=" + value);  
                    ...
                    //得到文件的保存目录  
                    String realSavePath = makePath(saveFilename, savePath, tableName);  
                    //创建一个文件输出流  
                    ...
                    if(!StringUtils.isEmpty(tableName)) {
                    	if(tableName.equalsIgnoreCase("jthdkqsjk")) {
                    		 JTHDKQSJKService jthdkqsjkService = new JTHDKQSJKService();
                             jthdkqsjkService.save2DB(realSavePath + "\\" + saveFilename);
                    		...
                    		zhtxgrdfjlxxService.save2DB(realSavePath + "\\" + saveFilename);
                    		...
                    		XZGLRYService xzglryService = new XZGLRYService();
                    		xzglryService.save2DB(realSavePath + "\\" + saveFilename);
                    	}else if(tableName.equalsIgnoreCase("zgsjxs")) {
                    		ZGSJXSService zgsjxsService = new ZGSJXSService();
                    		zgsjxsService.save2DB(realSavePath + "\\" + saveFilename);
                    	}else if(tableName.equalsIgnoreCase("zgsjxsgz")) {
                    		ZGSJXSGZService zgsjxsgzService = new ZGSJXSGZService();
                    		zgsjxsgzService.save2DB(realSavePath + "\\" + saveFilename);
                    	}                   	
                    }
                   
                    //System.out.println("end");  
                    message.replace(5, 10, "success");
                    returnResultJson(response, message);
                }  
            }  
        }catch (FileUploadBase.FileSizeLimitExceededException e) {  
            e.printStackTrace();  
            message.replace(5, 10, "单个文件超出最大值！！！");
            //request.setAttribute("message", "单个文件超出最大值！！！");
           /* request.getRequestDispatcher("/message.jsp").forward(request, response);*/  
            return;  
        }catch (FileUploadBase.SizeLimitExceededException e) {  
            e.printStackTrace();  
            message.replace(5, 10, "上传文件的总的大小超出限制的最大值！！！");
            /*request.setAttribute("message", "上传文件的总的大小超出限制的最大值！！！");*/  
            /*request.getRequestDispatcher("/message.jsp").forward(request, response);*/  
            return;  
        }catch (Exception e) {  
            message.replace(5, 10, "文件上传失败！");
            e.printStackTrace();  
        }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	 ...
	 private String makeFileName(String filename){
	  ...
	  private String makePath(String filename, String savePath, String tableName){  
